...
Data exploration techniques such as pairwise plots proved not very helpful with this data, given the large number of variables and difficulty in interpreting their influence. Therefore for our model selection we relied heavily on the original paper (@vello2013), which used random forests with bagging. With only random forest using default parameters, the model achieves 95% accuracy. Good enough for our purpose.  
...
Those variables are removed, leaving 52 predictors (down from 159).
...
Finally, the prediction and confusion matrix for the validation data suggests we can expect about 95% accuracy for out-of-sample data.
